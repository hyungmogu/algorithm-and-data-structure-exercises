...
def solution(A, B):
    ...
    fishes_to_upstream_stack = Stack()
    fishes_alive_count = 0
    N = len(A)

    index = N - 1

    while index >= 0:
        # if fish going upstream, add to stack
        if B[index] == 0:
            fishes_to_upstream_stack.push(A[index])

        # if fish going downstream, compare
        if B[index] == 1:
            fish_downstream = A[index]

            fish_upstream = fishes_to_upstream_stack.pop()

            if fish_upstream is None:
                fishes_alive_count += 1
            elif fish_upstream > fish_downstream:
                fishes_to_upstream_stack.push(fish_upstream)
            else:
                continue

        index -= 1

    fishes_alive_count += len(fishes_to_upstream_stack.items)

    return fishes_alive_count
