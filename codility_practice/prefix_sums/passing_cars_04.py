...
def solution(A):
    ...
    passing_cars_total = 0
    cars_to_west_total = sum(A)

    for value in A:
        if value == 0:
            passing_cars_total += cars_to_west_total

        if value == 1:
            cars_to_west_total -= 1

        if passing_cars_total > UPPER_LIMIT:
            return -1

    return passing_cars_total