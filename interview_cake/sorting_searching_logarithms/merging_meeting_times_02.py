...
class Solution:
    def mergeMeetingTimes(self, schedules):
        ...
        while index < len(schedules):
            ...
            if self.scheduleMergeable(schedules, index):
                if self.mergeableWithLastMergedSchedule(schedules, merged_schedule, index):
                    merged_schedule[-1] = (merged_schedule[-1][0], max(merged_schedule[-1][1], schedules[index][1]))
                else:
            ...
            #   2.2 if not 2.1 append schedules[index-1] to merged_schedule
            if not self.scheduleMergeable(schedules, index) and index == 1:
                merged_schedule.append(schedules[index-1])

            #   2.3 if not 2.1 and at the end of array, append schedules[index] to merged_schedule
            if not self.scheduleMergeable(schedules, index):
    ...
    def scheduleMergeable(self, schedules, index):
        if schedules[index][0] >= schedules[index-1][0] and schedules[index][0] <= schedules[index-1][1]:
            return True

        return False

    def mergeableWithLastMergedSchedule(self, schedules, merged_schedule, index):
        if schedules[index][0] >= merged_schedule[-1][0] and schedules[index][0] <= merged_schedule[-1][1]:
            return True
